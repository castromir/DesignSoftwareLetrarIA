...
export function AdminDashboard({ user, onLogout }: AdminDashboardProps) {
  ...
  const handleAddProfessional = async (e: React.FormEvent) => {
    ...
    try {
      await createProfessional({
        ...
        email:
          newProfessional.email || `${newProfessional.username}@letraria.com`,
  ...
  const handleSaveEdit = async (
    ...
    try {
      ...
      if (updatedProfessional.email)
        updateData.email = updatedProfessional.email;
      if (updatedProfessional.function !== undefined)
        updateData.function = updatedProfessional.function;
      if (updatedProfessional.username)
        updateData.username = updatedProfessional.username;
      if (updatedProfessional.password)
        updateData.password = updatedProfessional.password;

      // handle status / active fields (accept Portuguese 'Ativo'/'Inativo' or 'active'/'inactive')
      if ((updatedProfessional as any).status !== undefined) {
        const s = String((updatedProfessional as any).status).toLowerCase();
        if (s === "ativo" || s === "active") {
      ...
      } else if ((updatedProfessional as any).active !== undefined) {
        updateData.active = (updatedProfessional as any).active;
  ...
  const handleDeleteProfessional = async (id: string) => {
    ...
    if (
      window.confirm(
        `Tem certeza que deseja remover o profissional ${prof.name}? Esta ação não pode ser desfeita.`,
      )
    ) {
      try {
        setSuccessMessage("");
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b sticky top-0 z-10 shadow-[0px_1px_3px_0px_rgba(0,0,0,0.1),0px_1px_2px_-1px_rgba(0,0,0,0.1)]">
        <div className="px-4 sm:px-6 lg:px-[167.5px] h-14 sm:h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div>
              <h1 className="text-[16px] sm:text-[20px] leading-5 sm:leading-[28px] text-[#155dfc]">
                Letrar IA
              </h1>
              <p className="text-[10px] sm:text-[12px] leading-3 sm:leading-4 text-[#6a7282] hidden sm:block">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            <div className="text-right hidden md:block">
              <Badge
                variant="secondary"
                className="text-[12px] leading-4 bg-[#eceef2] text-[#030213] h-[22px]"
              >
      ...
      <main className="px-4 sm:px-6 lg:px-8 py-6 sm:py-8 max-w-[1280px] mx-auto">
        <div className="mb-6">
          <h2 className="text-[24px] sm:text-[30px] leading-8 sm:leading-9 mb-2">
            Gerenciar Profissionais da Educação
          </h2>
          <p className="text-[14px] sm:text-[16px] leading-5 sm:leading-6 text-[#4a5565]">
            Cadastre e gerencie os profissionais que terão acesso ao sistema
            Letrar IA
          </p>
        </div>
        ...
        {(successMessage || error) && (
          <Alert
            className={`mb-6 ${error ? "bg-red-50 border-red-200" : "bg-green-50 border-green-200"}`}
          >
            <AlertDescription
              className={error ? "text-red-800" : "text-green-800"}
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-6">
          <Card className="border-[rgba(0,0,0,0.1)]">
            <CardHeader className="p-4 sm:p-6">
              <CardDescription className="text-[14px] sm:text-[16px] leading-5 sm:leading-6 text-[#717182]">
                Total de Profissionais
              </CardDescription>
              <CardTitle className="text-[28px] sm:text-[36px] leading-8 sm:leading-10">
                {stats.total}
              </CardTitle>
            </CardHeader>
          </Card>
        ...
        <Card className="border-[rgba(0,0,0,0.1)]">
          <div className="p-4 sm:p-[25px]">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 sm:mb-[30px]">
              <div>
                <h3 className="text-[14px] sm:text-[16px] leading-4 mb-1">
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-b border-[rgba(0,0,0,0.1)]">
                    ...
                    <TableHead className="text-[14px] leading-5 h-10 text-right">
                      Ações
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {professionals.length === 0 ? (
                    ...
                    professionals.map((professional) => {
                      const isActive =
                        (professional as any).active !== undefined
                          ? (professional as any).active
                          : (professional as any).status !== undefined
                            ? String(
                                (professional as any).status,
                              ).toLowerCase() !== "inativo" &&
                              String(
                                (professional as any).status,
                          ...
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-2">
                              <button
                                ...
                                className="h-8 w-9 cursor-pointer rounded-lg hover:bg-gray-100 flex items-center justify-center transition-colors"
                              >
                                <Edit className="h-4 w-4 hover:opacity-90" />
                              </button>
                              <button
                                onClick={() =>
                                ...
                                className="h-8 w-9 cursor-pointer rounded-lg  hover:bg-gray-100 flex items-center justify-center transition-colors"
                              >
                                <Trash2 className="h-4 w-4 text-[#FB2C36] hover:opacity-90" />
                              </button>
                            </div>
                          </TableCell>
                        </TableRow>
                ...
                professionals.map((professional) => {
                  const isActive =
                    (professional as any).active !== undefined
                      ? (professional as any).active
                      : (professional as any).status !== undefined
                        ? String((professional as any).status).toLowerCase() !==
                            "inativo" &&
                          String((professional as any).status).toLowerCase() !==
                            "inactive"
                        : true;
                  return (
                    <div
                      key={professional.id}
                      className="border border-[rgba(0,0,0,0.1)] rounded-lg p-4 space-y-3"
                    >
          ...
          >
            <div>
              ...
              <div className="space-y-4 sm:space-y-[18px]">
                ...
                <div className="space-y-2">
                  <Label
                    htmlFor="prof-function"
                    className="text-[14px] text-neutral-950"
                  >
                    Função/Cargo
                  </Label>
                  <Select
                    value={newProfessional.function}
                    onValueChange={(value) =>
                      setNewProfessional({
                        ...newProfessional,
                        function: value,
