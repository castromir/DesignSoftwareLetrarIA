...
class ProfessionalService:
    ...
    async def list_professionals(self, skip: int = 0, limit: int = 100) -> dict:
        professionals = await self.professional_repository.get_all(
    ...
    async def update_professional(
        self, professional_id: str, data: ProfessionalUpdate
    ) -> dict:
        if data.email:
            existing_user = await self.user_repository.get_by_email(data.email)
        ...
        professional = await self.professional_repository.update(
            uuid.UUID(professional_id), update_data
        )
    ...
    async def delete_professional(self, professional_id: str) -> bool:
        # The database has ON DELETE CASCADE configured for students.professional_id,
        # so when we delete the professional, all associated students will be automatically
        # deleted by the database. No need for manual soft-delete.
